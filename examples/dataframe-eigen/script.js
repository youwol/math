...
function check(stresses) {
    ...
    evectors.forEach((stress) => {
        ...
        if (Math.abs(c1) > tol) {
            console.error('S1 and S2 are NOT ortho')
        }
        if (Math.abs(c2) > tol) {
            console.error('S1 and S3 are NOT ortho')
        }
        if (Math.abs(c3) > tol) {
            console.error('S2 and S3 are NOT ortho')
        }
    })
}
...
const grid = io.decodeGocadTS(
    fs.readFileSync(
        '/Users/fmaerten/data/arch/mud/simulations-grid.ts',
        'utf8',
    ),
)[0]
